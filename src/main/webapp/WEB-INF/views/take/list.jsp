...
<body>
	...
	<div id="take_list_wrap">
		<div id="left_wrap">
			<div id="left_top">
				<div id="search_wrap">
					...
					<form method="get" action="">
						...
						</span> <span> <input id="search_text" type="text" name="keyword"
							placeholder="지역을 입력해 주세요" value="${pager.keyword}">
				...
				<div id="selectBtn">
					<a href="list" class="listBtn1">돌봄</a> <a href="wklist"
						class="wklistBtn1">산책</a>
				</div>
			</div>
			...
			<div id="take_list">
						...
						<c:if test="${item.serviceSortId == 1}">
							<div class="take_cardList">
								<ul class="take_list_content"
									onclick="location.href='${item.serviceId}/detail'">
									<li class="take_cardList_img"><img src="/take/${item.serviceCover}" alt=""></li>									
									<li class="take_list_content_text">									
										<p>작성자: ${item.usrNickname}</p>
										<p>주소: ${item.usrAddr}</p>
										<p><fmt:formatNumber value="${item.serviceSittingPrice}" pattern="#,###"/>원</p>										
										<p>${item.servicePickup}</p> 
										<c:if test="${item.usrId == sessionScope.user.usrId}">
										<p class="btn_wrap">
											<a href="${item.serviceId}/update" class="updateBtn"><img src="/image/update.png"></a> 
											<a href="${item.serviceId}/delete" class="deleteBtn"><img src="/image/delete.png"></a>
										</p>
										</c:if>
									</li>
								</ul>
				...
				<div class="page">
					...
					<ul class="page_ul">
						<li class="page_li"><a
							href="?page=${pager.prev}&${pager.query}"><img
								src="/image/page_left.png"></a></li>
						<c:forEach var="page" items="${pager.list}">
							<li class="${page == pager.page ? 'page-item active' : ''}"><a
								href="?page=${page}&${pager.query}">${page}</a></li>
						</c:forEach>
						<li class="page_li"><a
							href="?page=${pager.next}&${pager.query}"><img
								src="/image/page_right.png"></a></li>
					</ul>
				</div>
				<!-- 페이지넨션 -->
			</div>
		...
		<div id="right_wrap">
			<div id="addBtn">
				<div id="addBtn_wrap">
					<img src="/image/add.png"><a href="add">서비스 등록하기</a>
				</div>
			</div>
						...
						<script>
							...
							function placesSearchCB(data, status, pagination) {
								...
								var iwContent = '<div class="infoWrap">'
								               ...
								               + '<div class="infoPrice">가격: ${item.serviceWalkingPrice}원</div>'
									           + '</div>'
									           + '<div class="infoA"><a href="${item.serviceId}/detail">상세보기</a></div>'
										       + '</div>'
											   + '</div>';
