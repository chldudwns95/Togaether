...
<head>
<meta charset="UTF-8">
<link href="/css/takeDetail.css" rel="stylesheet" type="text/css" />
...
<body>
   <jsp:include page="../nav.jsp"></jsp:include>
    ...
    <div id="takeDetail_wrap">
        <div class="takeDetail_serviceImg">
        ...
        <div class="takeDetail_service_usrInfo">
            ...
            <div>
                <p class="takeDetail_service_usrInfo_nick">${item.usrNickname } 님</p>
                <div><p>${item.serviceHouseYard }</p><p>${item.servicePickup }</p></div>
            </div>
            <div class="takeDetail_serviceInfo_wrap">
                <div class="price"><img src="/image/priceIcon.png"><p><fmt:formatNumber value="${item.serviceSittingPrice }" pattern="#,###"/></p></div>
                <a href="/book/${serviceId}/add">예약하기</a>
            </div>
        </div>
        ...
        <div class="takeDetail_content_wrap">
            <div class="takeDetail_left_wrap">
                <div class="date">
                    <div><p>시작 가능 날짜</p><p>${item.serviceStartDate.substring(0,10) }</p></div>
                    <div><p>종료 날짜</p><p>${item.serviceEndDate.substring(0,10) }</p></div>
                </div>
                <div class="house">
                    <div><p>주택종류</p><p>${item.serviceHouseSort }</p></div>
                    <div><p>주택크기</p><p>${item.serviceHouseSize }</p></div>
                </div>
                <div><p>${item.usrNickname } 님을 소개합니다</p><p>${item.serviceEtc }</p></div>
                <div><p>추가서비스</p>
                <c:forEach var="extra" items="${extra}">
                ${extra.extraContent}
                </c:forEach>
                </div>
            </div>
                   ...
                   <div class="takeDetail_history_content">
                       <div>
                           <div class="takeDetail_history_usrCover">
                              <c:if test="${history.usrCover ne null}">
